...
export default class DocElementPanel {
    ...
    render() {
        ...
        elDiv.append(`<label for="rbro_doc_element_position">${this.rb.getLabel('docElementPosition')}:</label>`);
        let elFormField = $('<div class="rbroFormField rbroSplit"></div>');
        let elPosX = $(`<input id="rbro_doc_element_position_x">`)
            .on('input', event => {
                let obj = this.rb.getDataObject(this.selectedObjId);
                if (obj !== null && obj.getValue('x') !== elPosX.val()) {
                    let cmd = new SetValueCmd(this.selectedObjId, 'rbro_doc_element_position_x', 'x',
        ...
        let elPosY = $('<input id="rbro_doc_element_position_y">')
            .on('input', event => {
                let obj = this.rb.getDataObject(this.selectedObjId);
                if (obj !== null && obj.getValue('y') !== elPosY.val()) {
                    let cmd = new SetValueCmd(this.selectedObjId, 'rbro_doc_element_position_y', 'y',
    ...
    show(data) {
        $('#rbro_doc_element_panel').removeClass('rbroHidden');
        this.updateData(data);
        this.updateAutosizeInputs();
    }

    static hide() {
        $('#rbro_doc_element_panel').addClass('rbroHidden');
    }
    ...
    updateData(data) {
        if (data !== null) {
            $('#rbro_doc_element_position_x').prop('disabled', false);
            $('#rbro_doc_element_position_y').prop('disabled', false);
            $('#rbro_doc_element_width').prop('disabled', false);
            $('#rbro_doc_element_height').prop('disabled', false);
            $('#rbro_doc_element_color').spectrum('enable');
            $('#rbro_doc_element_print_if').prop('disabled', false);

            $('#rbro_doc_element_position_x').val(data.getValue('x'));
            $('#rbro_doc_element_position_y').val(data.getValue('y'));
            $('#rbro_doc_element_width').val(data.getValue('width'));
            $('#rbro_doc_element_height').val(data.getValue('height'));
            $('#rbro_doc_element_color').spectrum("set", data.getValue('color'));
            $('#rbro_doc_element_print_if').val(data.getValue('printIf'));
            this.selectedObjId = data.getId();
        } else {
            $('#rbro_doc_element_position_x').prop('disabled', true);
            $('#rbro_doc_element_position_y').prop('disabled', true);
            $('#rbro_doc_element_width').prop('disabled', true);
            $('#rbro_doc_element_height').prop('disabled', true);
            $('#rbro_doc_element_color').spectrum('disable');
            $('#rbro_doc_element_print_if').prop('disabled', true);
        }
